...
public interface UserRepository extends JpaRepository<User, Long> {
    ...
    @Query("SELECT DISTINCT u FROM User u LEFT JOIN FETCH u.completedTasks LEFT JOIN FETCH u.unlockedRewards LEFT JOIN FETCH u.group WHERE u.id = :userId")
    Optional<User> findByIdWithCollections(@Param("userId") Long userId);
    
    @Query("SELECT DISTINCT u FROM User u LEFT JOIN FETCH u.group WHERE u.id = :userId")
    Optional<User> findByIdWithGroup(@Param("userId") Long userId);
