...
import com.example.demo.service.TaskGuideService;
...
public class AdminController {
    ...
    @PostMapping("/task-reviews/approve-all")
    public ResponseEntity<?> approveAllTasks(HttpSession session) {
        Long adminId = (Long) session.getAttribute("adminId");
        if (adminId == null) {
        ...
        try {
            ...
            for (UserTask userTask : pendingTasks) {
                try {
                    userTaskService.approveTask(userTask.getId(), adminId, "Batch approved by admin");
                    approvedCount++;
                } catch (Exception e) {
                    System.err.println("Error approving task " + userTask.getId() + ": " + e.getMessage());
            ...
            return ResponseEntity.ok(Map.of(
                "success", true,
                "message", "Successfully approved " + approvedCount + " tasks",
                "count", approvedCount
            ));
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body(Map.of("error", "Error approving tasks: " + e.getMessage()));
        }
    }
    
    // ==================== Task Guide Management Endpoints ====================
    
    @Autowired
    private TaskGuideService taskGuideService;

    /**
     * Get task guide data for a specific task
     */
    @GetMapping("/greenverse/tasks/{taskId}/guide")
    @ResponseBody
    public ResponseEntity<?> getTaskGuide(@PathVariable Long taskId, HttpSession session) {
        Long adminId = (Long) session.getAttribute("adminId");
        if (adminId == null) {
     ...
     * Delete task guide
     */
    @DeleteMapping("/greenverse/tasks/{taskId}/guide")
    @ResponseBody
    public ResponseEntity<?> deleteTaskGuide(@PathVariable Long taskId, HttpSession session) {
        Long adminId = (Long) session.getAttribute("adminId");
        if (adminId == null) {
            return ResponseEntity.status(401).body(Map.of("error", "Not authenticated"));
        }
